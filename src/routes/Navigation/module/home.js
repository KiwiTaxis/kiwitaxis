...
setInterval(updatelocation,20000)   
            ...
            { 
                ...
                return(dispatch,store)=>{
                    ...
                    .query({
                      ...
                      .finish((error, res)=>{
                        console.log (res);  
                      if(res.text.errMsg=="Success" &&  payload == "accept")
                      {  
                        dispatch({
                        ...
                        payload: res.body.data.trip_id
                          ...
                          .query({
                              ...
                              origin: store().navigation.region.latitude  + "," +  store().navigation.region.longitude,
                              destination: store().home.newrequest.dest.latitude  + "," +  store().home.newrequest.dest.longitude,
                              mode: "driving",
                              key:"AIzaSyBWke_00RksOBNheCG6v3HSeF8UsGyZQus"
                          })
                          .finish((error, res)=>{
    ...
    function handleGetDistanceTime(state,action){
        return update(state,{
            ...
            export function getarrivalstatus(payload){
                console.log(payload);
                if(payload.value === "arrived"){
                return(dispatch,store)=>{
                         dispatch({
                                    type:"GET_ARRIVAL_STATUS",
                                    payload :payload.value
                                })
                        }
                    }else if(payload.value === "starttrip"){

                        return(dispatch,store)=>{
                            let trip_id = store().navigation.trip_id;
                           
                  Promise.resolve(payload.token).then(function(values) {
                     request.post('http://54.252.173.49/api/driver/startTrip')
                     .query({
                        api_token : values,
                        trip_id:  trip_id
                       })
                       .finish((error, res)=>{
                           ...
                           if(res.body.errMsg === "success")
                           {
                            ...
                            .query({
                                ...
                                origin: store().navigation.newrequest.src.latitude  + "," +  store().navigation.newrequest.src.longitude,
                                destination: store().navigation.newrequest.dest.latitude  + "," +  store().navigation.newrequest.dest.longitude,
                                mode: "driving",
                                key:"AIzaSyBWke_00RksOBNheCG6v3HSeF8UsGyZQus"
                            })
                            .finish((error, res)=>{
                                ...
                                setTimeout(function(){
                                    ...
                                    var polyline = store().navigation.distancetimepolyline.routes[0].overview_polyline.points;
                       ...
                       dispatch({
                        type: "START_TRIP",
                        lat:  store().navigation.newrequest.src.latitude,
                        lng: store().navigation.newrequest.src.longitude
                 })
                }
                else{
                    dispatch({
            ...
            function handlearrivalstatus(state,action){
    ...
    function handlestarttrip(state,action)
    {  return update(state, {
        changedregion:{
            latitude:{
                $set: action.lat
    ...
    export function finishtrip(payload){
                    return(dispatch,store)=>{
                        let distance = store().navigation.distancetimepolyline.routes[0].legs[0].distance.value;
                        let dinkm = distance/1000;

                        Promise.resolve(payload.token).then(function(values) {
                        request.post('http://54.252.173.49/api/driver/finishTrip')
                        .send({
                                api_token : values,
                                trip_id:  store().navigation.trip_id,
                                distance : dinkm
                        })
                        .finish((error, res)=>{
                         alert(res);
            ...
            function handlefinishtrip(state,action)
            {  
                return update(state, 
                {
                    setrating:{
            ...
            export function submitRating(payload){
                return(dispatch,store)=>{
                    let tripid = store().navigation.trip_id;
                    let rate  = store().navigation.rating;
                    console.log(rate);
                    Promise.resolve(payload).then(function(values)
                    {
                        
                        request.post('http://54.252.173.49/api/driver/submitReview')
                        .send({
                                api_token : values,
                                trip_id:tripid,
                                rate : rate,
            ...
            function handleSubmitrating(state,action){
                ...
                export function ratingCompleted(payload){
                    return(dispatch,store)=>{
                        dispatch({
                            type:"SET_RATING",
                            payload
            ...
            function handleRating(state,action)
            {
                return update(state,{
                    rating: {
                        $set:action.payload
               ...
               const ACTION_HANDLERS = {
                ...
                GET_ARRIVAL_STATUS : handlearrivalstatus,
                START_TRIP:handlestarttrip,
                FINISH_TRIP:handlefinishtrip,
                SUBMIT_RATING :handleSubmitrating,
                SET_RATING :handleRating   
                };
